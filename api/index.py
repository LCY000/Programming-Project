...
# 獲得圖文選單識別碼                
rich_menu_id = line_bot_api.get_rich_menu_list()
...
def handle_normal_state(user_id, user_message, event):
    ...
    if user_message == '新增 待辦事項':
        ...
        # 關閉圖文選單
        line_bot_api.unlink_rich_menu_from_user(user_id,rich_menu_id, timeout=5)
        # 還需製作一個取消新增訊息的功能......
...
def handle_message(event):
    ...
    if state == UserState.ADD_TODO:
            ...
            user_state[user_id] = UserState.NORMAL
            # 開啟圖文選單
            line_bot_api.link_rich_menu_to_user(user_id, rich_menu_id, timeout=5)
